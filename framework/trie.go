...
// 判断一个segment是否时通用segment，即以:开头
func isWidSegment(segment string) bool {
...
func (n *node) filterChildNodes(segment string) []*node {
	...
	if isWidSegment(segment) {
		return n.children
	}
	nodes := make([]*node, 0, len(n.children))
	// 过滤所有的下一层子节点
	for _, child := range n.children {
		if isWidSegment(child.segment) {
